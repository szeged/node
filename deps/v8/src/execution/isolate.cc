...
static void pattern_add(char *input, std::vector<char*>* patterns)
{
  ...
  while (true) {
    ...
    if (start < input) {
      ...
      pattern = (char*)Malloced::operator new(size + 1);
    ...
    JSFunction function, SharedFunctionInfo shared)
{
  ...
  Script::PositionInfo info;
  script.GetPositionInfo(shared.StartPosition(), &info, Script::WITH_OFFSET);

  int line = info.line + 1;
  int col = info.column + 1;
  char *final_str;

  if (line == 1 && script.IsModule()) {
    // Module.
    if (col == 1) {
      col = 0;
    } else if (col < 62) {
  ...
  if (script_name_raw.IsString()) {
    ...
    final_str = (char*)Malloced::operator new(info_size);
  ...
  if ((isolate->_include.size() > 0 && !pattern_match(final_str, &isolate->_include))
    ...
    Malloced::operator delete(final_str);
    shared.set_trace_id(-1);
    return -1;
  }
